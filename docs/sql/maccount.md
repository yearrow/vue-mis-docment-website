...
|item_type| INT | 合同类型 | | |
|item_data_type| INT | 选材方式 | | |
|item_data_id | BIGINT | 选材来源ID | |  |
...
|isAudit| BOOLEAN | 判断变更合同是不是审批或审核 | | |
|sort_code| INTEGER | 排序 | | |
|is_removed| bool | 删除状态 | | |
|creator_id| bigint | 添加人 | | |
|creator_name| string | 添加人姓名 | | |
...
|material_type| STRING | 材料类型（新增） | | |
|material_type_code| STRING | 材料类型code（新增） | | |
|labour_name | STRING | 用料单位名称| | |
|labour_id | BIGINT | 用料单位Id| | |
|gh_name | STRING | 工号名称| | |
|gh_id | BIGINT | 工号id| | |
...
|ori_labour_id| STRING | 对接第三方用料单位 | | |
|store_room_name | STRING | 成本科目（暂未用）| | |
|store_room_full_name | STRING | 成本科目全称（暂未用）| | |
|store_room_id | BIGINT | 成本科目Id（暂未用）| | |
...
|updated_at| timestamp | 修改时间 | | |
|version| bigint | 版本号 | | |
|inventory_id| BIGINT | 库存主键id | | |
|store_id| BIGINT | 来源ID | | |
|refund_delivery_quantity| DECIMAL(28, 4) | 退发量（暂未用） | | |
|refund_delivery_price| DECIMAL(28, 8) | 退发价（暂未用） | | |
|refund_delivery_sum| DECIMAL(28, 4) | 退发金额（暂未用） | | |
...
|use_quantity | DECIMAL(28, 4) | 应耗量| | |
|jc_quantity | DECIMAL(28, 4) | 节超| | |
|ori_labour_id| STRING | 【常用用料单位】预留 | | |
